...
```Request``` has two methods: ```readRequest()``` which returns a ```std::string``` containing the contents of the request, and ```respond(const std::string& message)``` which responds to the client conenction with the given message.
